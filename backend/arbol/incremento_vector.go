...
func (i Incremento_vector) Ejecutar(ambito *ambito.Ambito) interface{} {
	...
	if encontrado != nil {
		...
		case string:
			if encontrado.Primitivo == "String" {
				if encontrado.Referencia {
					encontrado.Puntero_valor.Lista_vector[indice] = encontrado.Puntero_valor.Lista_vector[indice].(string) + rr
					return nil
				}
				encontrado.Lista_vector[indice] = encontrado.Lista_vector[indice].(string) + rr
				return nil
			}
		default:
